function wsFeat = kitMocFeat(src, wsMoc, parF, varargin)
% Obtain feature from the Kitchen mocap source.
%
% Input
%   src      -  kit src
%   wsMoc    -  mocap data
...
if svL == 2 && exist(path, 'file')
    prInOut('kitMocFeat', 'old, src %s, setNm %s', src.nm, setNm);
    wsFeat = matFld(path, 'wsFeat');
    return;
end
prIn('kitMocFeat', 'new, src %s, setNm %s', src.nm, setNm);
